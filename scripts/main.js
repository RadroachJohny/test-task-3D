...
//   if (arr[0] === '-'){   
//     arr = arr.slice(0,1) + parseInt((arr.slice(1).reverse()).join(''));
//     return parseInt(arr);
//   }
...
//   if (walk.length === 10 && walk[0] !== walk[walk.length-1] && Object.values(elemsObj)[0] === Object.values(elemsObj)[1]){
//     return true;
//   }
...
function f(clockArr, money) {
  ...
  for (let i = 0; i < clockArr.length - 1; i++) {    
    ...
    clockArr.some((elem) => {
        if(Math.round(clockArr[i] + elem) === Math.round(money) && clockArr[i] !== elem) {
          pos = true;
          return;
        }
    });
...
console.log(f([20.00, 18.51, 97.75, 35.20, 76.42, 54.10], 89.3));

console.log(Math.round(54.10 + 35.20) === Math.round(89.3));

console.log(parseInt(0.000005)); //0
console.log(parseInt(0.5));//0
console.log(parseInt(0.0000005));//5

console.log(typeof (0.1 + 0.2).toFixed(1)); //0.3
